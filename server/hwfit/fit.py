...
"""Hardware-fit scoring: GPU bandwidth table, tok/s estimator, model ranking.

Ported near-verbatim from Odysseus services/hwfit/fit.py. The GPU_BANDWIDTH
table and the offload speed model are empirically calibrated (RX 9060 XT:
~59 t/s estimated vs 59.8 measured) — intentionally unchanged. Differences
from the original: package-relative imports; image-model fit is NOT ported
(Odysseus services/hwfit/image_models.py, v2+); and one additive gap-fill —
GTX 10-series rows in GPU_BANDWIDTH (spec-sheet memory bandwidths: 1080 Ti
484 GB/s, 1080 320, 1070 Ti/1070 256, 1060 192), since the original table
starts at the 16-series and Pascal cards otherwise fall to FALLBACK_K."""
import re
...
GPU_BANDWIDTH = {
    ...
    "1080 ti": 484, "1080": 320, "1070 ti": 256, "1070": 256, "1060": 192,
...
def rank_models(system, use_case=None, limit=50, search=None, sort="score", quant=None, target_context=None, fit_only=False):
    ...
    # Image-generation fit (Odysseus services/hwfit/image_models.py) is NOT
    # ported — text-model serving only in SERVARI v1. Return an empty list
    # rather than mis-ranking text models under an image_gen filter.
    ...
    for m in models:
        ...
        # MLX needs the mlx_lm runtime, which SERVARI does not generate serve
        ...
        # otherwise prefer GGUF/Q* entries that can be routed through
        ...
        #
        # Windows is the same: the practical local path is llama.cpp, which
        # requires GGUF. AWQ/GPTQ models without a GGUF source are unservable
        # there.
